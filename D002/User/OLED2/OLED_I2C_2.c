...
#include "bsp_i2c_2.h"
 /**
  * @brief  I2C_2_WriteByte，向OLED寄存器地址写一个byte的数据
  * @param  addr：寄存器地址
	*					data：要写入的数据
  * @retval 无
  */
void I2C_2_WriteByte(uint8_t addr,uint8_t data)
{
 ...
 /**
  * @brief  WriteCmd_2，向OLED写入命令
  * @param  I2C_Command：命令代码
  * @retval 无
  */
void WriteCmd_2(unsigned char I2C_Command)//写命令
{
 ...
 /**
  * @brief  WriteDat_2，向OLED写入数据
  * @param  I2C_Data：数据
  * @retval 无
  */
void WriteDat_2(unsigned char I2C_Data)//写数据
{
...
void OLED_Init_2(void)
{
	...
	WriteCmd_2(0xff); //亮度调节 0x00~0xff
 ...
 /**
  * @brief  OLED_SetPos_2，设置光标
  * @param  x,光标x位置
	*					y，光标y位置
  * @retval 无
  */
void OLED_SetPos_2(unsigned char x, unsigned char y) //设置起始点坐标
{ 
	WriteCmd_2(0xb0+y);
 ...
 /**
  * @brief  OLED_Fill_2，填充整个屏幕
  * @param  fill_Data:要填充的数据
	* @retval 无
  */
void OLED_Fill_2(unsigned char fill_Data)//全屏填充
{
	unsigned char m,n;
	for(m=0;m<8;m++)
	{
...
void OLED_CLS_2(void)//清屏
{
 ...
 /**
  * @brief  OLED_ON_2，将OLED从休眠中唤醒
  * @param  无
	* @retval 无
  */
void OLED_ON_2(void)
{
	WriteCmd_2(0X8D);  //设置电荷泵
	WriteCmd_2(0X14);  //开启电荷泵
	WriteCmd_2(0XAF);  //OLED唤醒
}


 /**
  * @brief  OLED_OFF_2，让OLED休眠 -- 休眠模式下,OLED功耗不到10uA
  * @param  无
	* @retval 无
  */
...
{
	WriteCmd_2(0X8D);  //设置电荷泵
	WriteCmd_2(0X10);  //关闭电荷泵
	WriteCmd_2(0XAE);  //OLED休眠
}


 /**
  * @brief  OLED_ShowStr_2，显示codetab.h中的ASCII字符,有6*8和8*16可选择
...
void OLED_ShowStr_2(unsigned char x, unsigned char y, unsigned char ch[], unsigned char TextSize)
{
 ...
 /**
  * @brief  OLED_ShowCN_2，显示codetab.h中的汉字,16*16点阵
  * @param  x,y: 起始点坐标(x:0~127, y:0~7); 
	*					N:汉字在codetab.h中的索引
	* @retval 无
  */
