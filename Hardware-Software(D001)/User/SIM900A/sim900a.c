...
{
	u8 res=0; 
	USART4_RX_STA=0;USART4_RX_REC_ATCOMMAD=1;
	if((u32)cmd<=0XFF)
	{
	...
	USART4_RX_STA=0;USART4_RX_REC_ATCOMMAD=0;
	return res;
} 
...
u8 sim900a_work_test(void)
{
	...
	{
		if(strstr((const char*)USART4_RX_BUF,"0,5")==NULL)
		{
			 if(!sim900a_send_cmd((u8 *)"AT+CSQ",(u8 *)"OK",200))	
			 {
					memcpy(SIM900_CSQ,USART4_RX_BUF+15,2);
			 }
