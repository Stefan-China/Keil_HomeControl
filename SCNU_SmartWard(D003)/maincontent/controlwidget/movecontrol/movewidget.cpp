    ...
    ui(new Ui::MoveWidget)
{
    ...
    connect(ui->DoorSwitch,SIGNAL(clicked()),this,SLOT(Onclicked_Mouse()));
    ...
    connect(ui->LivingSwitch,SIGNAL(clicked()),this,SLOT(Onclicked_Mouse()));
...
void MoveWidget::Onclicked_Mouse()
{
    ...
    }else if(!QString::compare(identifer,"DoorSwitch")){
        isSucceed = connector.setDataToAliyun(identifer,!cloudData.DoorSwitch);
    ...
    }else if(!QString::compare(identifer,"LivingSwitch")){
        isSucceed = connector.setDataToAliyun(identifer,!cloudData.LivingSwitch);
...
void MoveWidget::freshCloudData()
{
    ...
    for(int i = 0; i<deviceList.size(); i++){
        ...
        if(!device.isEmpty()){
            ...
            }else if(!QString::compare(deviceName,"LivingSwitch")){
                cloudData.LivingSwitch = device.value("value").toInt();
                if(cloudData.LivingSwitch==1){
            ...
            }else if(!QString::compare(deviceName,"DoorSwitch")){
                cloudData.DoorSwitch = device.value("value").toInt();
                if(cloudData.DoorSwitch==1){
